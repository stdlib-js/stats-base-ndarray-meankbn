...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof meankbn, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( meankbn.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the arithmetic mean of a one-dimensional ndarray', function test( t ) {
	...
	v = meankbn( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 0.5, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = meankbn( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -4.5, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = meankbn( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = meankbn( [ vector( x, 1, 1, 0 ) ] );
